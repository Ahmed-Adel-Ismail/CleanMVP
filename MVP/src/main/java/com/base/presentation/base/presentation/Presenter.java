...
import com.base.presentation.annotations.interfaces.OnKeyDownHandler;
import com.base.presentation.annotations.interfaces.OnNavigationMenuItemClickHandler;
import com.base.presentation.annotations.interfaces.OnTouchEventsHandler;
...
import com.base.presentation.base.abstracts.features.OnKeyDownParams;
 ...
 * {@link OnTouchesHandler} : the {@link PresenterOnTouchesHandler} that will handle touches
 * from listeners<br>
 * {@link OnTouchEventsHandler} : the {@link PresenterOnTouchEventHandler} that will handle touches
 * from Activity's {@code onTouchEvent()}<br>
 * {@link OnKeyDownHandler} : the {@link PresenterOnKeyDownHandler} that will handle keys pressed
 * from Activity's {@code onKeyDown()}<br>
 ...
 * @see OnTouchEventsHandler
 * @see OnChecksChangedHandler
 * @see OnKeyDownHandler
...
public abstract class Presenter<P extends Presenter<P, V, M>, V extends ViewModel, M extends Model>
    ...
    private CommandExecutor<Long, MotionEvent> onTouchEventsExecutor;
    private CommandExecutor<Long, KeyEvent> onKeyDownExecutor;
    ...
    public Presenter() {
        ...
        this.onTouchEventsExecutor = new Executor<>();
        this.onKeyDownExecutor = new Executor<>();
    ...
    public void initialize(V viewModel) {
        ...
        this.onTouchEventsExecutor.putAll(createOnToucheEventsExecutor());
        this.onKeyDownExecutor.putAll(createOnKeyDownExecutor());
    ...
    private CommandExecutor<Long, Message> createMainCommandExecutor() {
        ...
        command = createOnTouchEventCommand();
        cmdExec.put((long) R.id.onTouchEvent, command);

        command = createOnKeyDownCommand();
        cmdExec.put((long) R.id.onKeyDown, command);
    ...
    private Command<Message, Void> createOnTouchEventCommand() {
        return new Command<Message, Void>() {
            @Override
            public Void execute(Message message) {
                MotionEvent event = message.getContent();
                onTouchEventsExecutor.execute((long) event.getAction(), event);
                return null;
            }
        };
    }

    private Command<Message, Void> createOnKeyDownCommand() {
        return new Command<Message, Void>() {
            @Override
            public Void execute(Message message) {
                OnKeyDownParams params = message.getContent();
                onKeyDownExecutor.execute((long) params.keyCode, params.keyEvent);
                return null;
            }
        };
    }
    ...
    private void clear() {
        ...
        onTouchEventsExecutor.clear();
        onKeyDownExecutor.clear();
    ...
    private CommandExecutor<Long, MotionEvent> createOnToucheEventsExecutor() {
        Executor<MotionEvent> executor = null;
        try {
            ClassAnnotationReader<OnTouchEventsHandler> reader;
            reader = new ClassAnnotationReader<>(OnTouchEventsHandler.class);
            Class<?>[] classes = reader.execute(this).value();
            executor = new ExecutorClassesInitializer(this).execute(classes);
        } catch (AnnotationNotDeclaredException e) {
            // do nothing
    ...
    private CommandExecutor<Long, KeyEvent> createOnKeyDownExecutor() {
        Executor<KeyEvent> executor = null;
        try {
            ClassAnnotationReader<OnKeyDownHandler> reader;
            reader = new ClassAnnotationReader<>(OnKeyDownHandler.class);
            Class<?>[] classes = reader.execute(this).value();
            executor = new ExecutorClassesInitializer(this).execute(classes);
        } catch (AnnotationNotDeclaredException e) {
            // do nothing
    ...
    public final void updateViewModel() {
        if (presenterUpdater != null) {
            presenterUpdater.updateViewModel();
        }
    }
