...
import com.base.presentation.exceptions.OnMediaPlayerErrorStoppedException;
 ...
 * (including {@link Presenter} classes), this class is intended to be a generic listener that
 * can be assigned to any type and triggers events for it, that's why it does not respect the
 * {@code Opened-Closed Principle}, since it's responsibility is to convert callbacks into different
 * types of events
 * <p/>
...
public class OnEventListener implements
    ...
    private void invokeOnNavigationItemSelected(MenuItem item, AbstractActivity activity) {
        ...
        activity.onUpdateWithSameAddress(event);
    }
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        Event event = new Event.Builder(R.id.onMediaPlayerCompletion)
                .message(message)
                .senderActorAddress(activity.getActorAddress())
                .build();
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        ...
        Message message = new Message.Builder()
                .id(mp.getAudioSessionId())
                .content(new OnMediaPlayerErrorParams(mp, what, extra))
                .build();

        Event event = new Event.Builder(R.id.onMediaPlayerError)
                .message(message)
                .senderActorAddress(activity.getActorAddress())
                .build();
        ...
        try {
            activity.onUpdateWithSameAddress(event);
        } catch (OnMediaPlayerErrorStoppedException e) {
            Logger.getInstance().error(activity.getClass(), "onError() stopped");
            return true;
        }
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        Event event = new Event.Builder(R.id.onMediaPlayerPrepared)
                .message(message)
                .senderActorAddress(activity.getActorAddress())
                .build();
