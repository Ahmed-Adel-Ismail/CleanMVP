...
import com.base.abstraction.system.GlobalBroadcastReceiver;
...
/**
 * a class that reads {@link BroadcastsHandler} annotation and setup the broadcast receiver
...
class BroadcastsAnnotationReader implements Command<App, Void> {
    ...
    public Void execute(final App app) {
        ...
        new FieldAnnotationScanner<BroadcastsHandler>(BroadcastsHandler.class) {
            @Override
            protected void onAnnotationFound(Field field, BroadcastsHandler annotation) {
                try {
        ...
        if (!executors.isEmpty()) {
            app.registerReceiver(new GlobalBroadcastReceiver(executors), intentFilter);
        }
    ...
    private void throwUnsupportedOperationException() {
        throw new UnsupportedOperationException("@" + BroadcastsHandler.class.getSimpleName()
